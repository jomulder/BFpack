...
!    integer, parameter :: rint = selected_int_kind(6)
    ...
    integer(rint), intent(in) :: P, numcorr, K, numG, samsize0, Njs(numG), Ntot, seed
    real(rdp), intent(in)   :: BHat(numG,K,P), sdHat(2*numG,P), CHat(numG,P,P), XtXi(numG,K,K), Ygroups(numG,Ntot,P), &
                               Xgroups(numG,Ntot,K)
    ...
    real(rdp)               :: BDraws(numG,K,P), sigmaDraws(numG,P), CDraws(numG,P,P), Ccan(P,P), Ds(P,P), dummyPP2(P,P), &
    ...
    integer(rint)             :: s1, g1, i1, corrteller, c1, c2, p1, p2, k1, errorflag, lower_int, median_int, upper_int, &
                               iseed
!
    !set seed
    iseed = seed
