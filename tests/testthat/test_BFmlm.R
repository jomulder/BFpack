...
BF1 <- BF(lm1,parameter="correlation",hypothesis="cyl_with_mpg<hp_with_mpg<hp_with_cyl<0;
...
lm1 <- lm(cbind(mpg,cyl,hp,qsec) ~ -1 + disp + wt + vsfac, data = mtcars)
BF1 <- BF(lm1,parameter="correlation",hypothesis="cyl_with_mpg_in_vsfac0 < cyl_with_mpg_in_vsfac1 &
